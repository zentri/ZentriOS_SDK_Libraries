...
{
    // Get first child of an {} or [], if any.
    if (tok->type == JSMN_OBJECT && jsmn_key_count(tok))
    {
        return tok + 1;
    }
    else if (tok->type == JSMN_ARRAY && jsmn_array_count(tok))
    {
        return tok + 1;
    }
    return NULL;
...
{
    int len = strlen(s);
    return jsmn_size(tok) == len && !strncmp(jsmn_value(tok, in_json), s, len);
}

const char* jsmn_copy_str(jsmntok_t *tok, char *json_data, char *buffer)
{
    const size_t len = tok->end - tok->start;
    strncpy(buffer, &json_data[tok->start], len);
    buffer[len] = 0;
    return buffer+len;
}
