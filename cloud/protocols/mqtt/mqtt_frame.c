 ...
 *               Interface functions
 ******************************************************/
zos_result_t  mqtt_frame_recv( mqtt_buffer_t *buffer, void *p_user )
{
    ...
    if ( NULL == buffer )
    {
        ...
        return mqtt_backend_connection_close( conn );
    }
    ...
    switch (type)
    {
        ...
        {
            mqtt_suback_arg_t   args;
            MQTT_LOG("Received SUBACK");
        ...
        default:
            MQTT_LOG("RECEIVED UNEXPECTED TYPE (%d)", type);
            return ZOS_ERROR;
    }
    return result;
}

zos_result_t  mqtt_frame_create( uint16_t max_size, mqtt_frame_t *frame, mqtt_socket_t *socket )
{
    zos_result_t ret;
...
zos_result_t mqtt_frame_delete( mqtt_frame_t *frame )
{
    return mqtt_network_delete_buffer( frame->start );
}

zos_result_t mqtt_frame_send( mqtt_frame_t *frame, mqtt_socket_t *socket )
{
	// ToDo: is the the best way to pass size, or frame->size is better set and used?
    return mqtt_network_send_buffer( frame->start, frame->buffer.data - frame->start, socket );
}

/******************************************************
 *               Connect frame
 ******************************************************/
zos_result_t mqtt_frame_put_connect( mqtt_frame_t *frame, const mqtt_connect_arg_t *args )
{
    ...
    if ( args->mqtt_version == MQTT_PROTOCOL_VER4 )
    {
    ...
    if ( args->mqtt_version == MQTT_PROTOCOL_VER4 )
    {
        ...
        MQTT_BUFFER_PUT_OCTET( &frame->buffer, MQTT_PROTOCOL_VER4 );
    }
    else
    {
        MQTT_BUFFER_PUT_STRING( &frame->buffer, MQTT_PACKET_CONNECT_STR_VER3, strlen(MQTT_PACKET_CONNECT_STR_VER3) );
        MQTT_BUFFER_PUT_OCTET( &frame->buffer, MQTT_PROTOCOL_VER3 );
    }
    ...
    MQTT_BUFFER_PUT_SHORT( &frame->buffer, args->keep_alive ); /* Keep alive for now */
...
zos_result_t mqtt_frame_get_connack( mqtt_frame_t *frame, mqtt_connack_arg_t *args )
{
    uint8_t type;
    uint8_t size;
...
zos_result_t mqtt_frame_put_publish( mqtt_frame_t *frame, const mqtt_publish_arg_t *args )
{
...
zos_result_t mqtt_frame_get_publish( mqtt_frame_t *frame, mqtt_publish_arg_t *args )
{
    uint8_t type;
    uint32_t size;
    ...
    assert(type == MQTT_PACKET_TYPE_PUBLISH );
...
zos_result_t mqtt_frame_put_puback( mqtt_frame_t *frame, const mqtt_puback_arg_t *args )
{
    /* A long value carrying string MQTT */
...
zos_result_t mqtt_frame_get_puback( mqtt_frame_t *frame, mqtt_puback_arg_t *args )
{
    uint8_t type;
    uint8_t size;
    ...
    MQTT_BUFFER_GET_OCTET( &frame->buffer, type );    /* Type-Reserved */
    type = ( type & 0xF0 ) >> 4;
    assert(type == MQTT_PACKET_TYPE_PUBACK );
    MQTT_BUFFER_GET_OCTET( &frame->buffer, size );
    assert(size == 2 );
    MQTT_BUFFER_GET_SHORT( &frame->buffer, args->packet_id );
    return ZOS_SUCCESS;
...
zos_result_t mqtt_frame_put_pubrec( mqtt_frame_t *frame, const mqtt_pubrec_arg_t *args )
{
    /* A long value carrying string MQTT */
    MQTT_BUFFER_PUT_4BIT( &frame->buffer, 0x2, 0, 0 );    /* Reserved */
...
zos_result_t mqtt_frame_get_pubrec( mqtt_frame_t *frame, mqtt_pubrec_arg_t *args )
{
    uint8_t type;
    uint8_t size;
    ...
    MQTT_BUFFER_GET_OCTET( &frame->buffer, type );    /* Type-Reserved */
    type = ( type & 0xF0 ) >> 4;
    assert(type == MQTT_PACKET_TYPE_PUBREC );
    MQTT_BUFFER_GET_OCTET( &frame->buffer, size );
    assert(size == 2 );
    MQTT_BUFFER_GET_SHORT( &frame->buffer, args->packet_id );
    return ZOS_SUCCESS;
...
zos_result_t mqtt_frame_put_pubrel( mqtt_frame_t *frame, const mqtt_pubrel_arg_t *args )
{
    /* A long value carrying string MQTT */
    MQTT_BUFFER_PUT_4BIT( &frame->buffer, 0x2, 0, 0 );    /* Reserved */
...
zos_result_t mqtt_frame_get_pubrel( mqtt_frame_t *frame, mqtt_pubrel_arg_t *args )
{
    uint8_t type;
    uint8_t size;
    ...
    MQTT_BUFFER_GET_OCTET( &frame->buffer, type );    /* Type-Reserved */
    type = ( type & 0xF0 ) >> 4;
    assert(type == MQTT_PACKET_TYPE_PUBREL );
    MQTT_BUFFER_GET_OCTET( &frame->buffer, size );
    assert(size == 2 );
    MQTT_BUFFER_GET_SHORT( &frame->buffer, args->packet_id );
    return ZOS_SUCCESS;
...
zos_result_t mqtt_frame_put_pubcomp( mqtt_frame_t *frame, const mqtt_pubcomp_arg_t *args )
{
    /* A long value carrying string MQTT */
    MQTT_BUFFER_PUT_4BIT( &frame->buffer, 0x2, 0, 0 );    /* Reserved */
...
zos_result_t mqtt_frame_get_pubcomp( mqtt_frame_t *frame, mqtt_pubcomp_arg_t *args )
{
    uint8_t type;
    uint8_t size;
    ...
    MQTT_BUFFER_GET_OCTET( &frame->buffer, type );    /* Type-Reserved */
    type = ( type & 0xF0 ) >> 4;
    assert(type == MQTT_PACKET_TYPE_PUBCOMP );
    MQTT_BUFFER_GET_OCTET( &frame->buffer, size );
    assert(size == 2 );
    MQTT_BUFFER_GET_SHORT( &frame->buffer, args->packet_id );
    return ZOS_SUCCESS;
...
zos_result_t mqtt_frame_put_subscribe( mqtt_frame_t *frame, const mqtt_subscribe_arg_t *args )
{
...
zos_result_t mqtt_frame_get_suback( mqtt_frame_t *frame, mqtt_suback_arg_t *args )
{
    uint8_t type;
    uint8_t size;
    ...
    MQTT_BUFFER_GET_OCTET( &frame->buffer, type );    /* Type-Reserved */
    type = ( type & 0xF0 ) >> 4;
    assert(type == MQTT_PACKET_TYPE_SUBACK );
    MQTT_BUFFER_GET_OCTET( &frame->buffer, size );
    assert(size == 3 );
    MQTT_BUFFER_GET_SHORT( &frame->buffer, args->packet_id );
...
zos_result_t mqtt_frame_put_unsubscribe( mqtt_frame_t *frame, const mqtt_unsubscribe_arg_t *args )
{
    uint32_t    size = ( uint32_t ) 2                                                            /* Packet identifier  */
                       + ( uint32_t ) ( args->topic_filter.len + sizeof(args->topic_filter.len)); /* will topic if sent */
...
zos_result_t mqtt_frame_get_unsuback( mqtt_frame_t *frame, mqtt_unsuback_arg_t *args )
{
    uint8_t type;
    uint8_t size;
    ...
    MQTT_BUFFER_GET_OCTET( &frame->buffer, type );    /* Type-Reserved */
    type = ( type & 0xF0 ) >> 4;
    assert(type == MQTT_PACKET_TYPE_UNSUBACK );
    MQTT_BUFFER_GET_OCTET( &frame->buffer, size );
    assert(size == 2 );
    MQTT_BUFFER_GET_SHORT( &frame->buffer, args->packet_id );
    return ZOS_SUCCESS;
...
zos_result_t mqtt_frame_put_disconnect( mqtt_frame_t *frame )
{
    MQTT_BUFFER_PUT_4BIT( &frame->buffer, 0, 0, 0 );    /* Reserved */
...
zos_result_t mqtt_frame_put_pingreq( mqtt_frame_t *frame )
{
    MQTT_BUFFER_PUT_4BIT( &frame->buffer, 0, 0, 0 );    /* Reserved */
