...
#define PLATFORM_STRING "(ZentriOS 3.4.0; AMW106)"
    STRING_HANDLE result;

    if (product == NULL)
    {
        result = STRING_construct(CLIENT_DEVICE_TYPE_PREFIX CLIENT_DEVICE_BACKSLASH IOTHUB_SDK_VERSION " " PLATFORM_STRING);
    }
    else
    {
        result = STRING_construct_sprintf(CLIENT_DEVICE_TYPE_PREFIX CLIENT_DEVICE_BACKSLASH IOTHUB_SDK_VERSION " %s " PLATFORM_STRING, product);
    }
...
static IOTHUB_CLIENT_LL_HANDLE_DATA* initialize_iothub_client(const IOTHUB_CLIENT_CONFIG* client_config)
{
    ...
    else
    {
        ...
        else
        {
            ...
            const char* device_key = client_config->deviceKey;
            const char* device_id = client_config->deviceId;
            const char* sas_token = client_config->deviceSasToken;
            ...
            else
            {
                ...
                if ((result->transportHandle = IoTHubTransport_MQTT_Common_Create(&lowerLayerConfig, NULL)) == NULL)
                {
            ...
            if (result != NULL)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ((result->deviceHandle = IoTHubTransport_MQTT_Common_Register(result->transportHandle, &deviceConfig, result, &(result->waitingToSend))) == NULL)
                        {
                            LogError("Registering device in transport failed");
                            IoTHubClient_Auth_Destroy(result->authorization_module);
                            IoTHubTransport_MQTT_Common_Destroy(result->transportHandle);
                            destroy_blob_upload_module(result);
                            tickcounter_destroy(result->tickCounter);
                            STRING_delete(product_info);
                            free(result);
                        ...
                        else
                        {
                            ...
                            if (IoTHubClient_LL_SetRetryPolicy(result, IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF_WITH_JITTER, 0) != IOTHUB_CLIENT_OK)
                            {
                                ...
                                IoTHubTransport_MQTT_Common_Unregister(result->deviceHandle);
                                IoTHubClient_Auth_Destroy(result->authorization_module);
                                IoTHubTransport_MQTT_Common_Destroy(result->transportHandle);
                                destroy_blob_upload_module(result);
                                tickcounter_destroy(result->tickCounter);
...
{
    return initialize_iothub_client(config);
}
...
void IoTHubClient_LL_Destroy(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle)
{
    ...
    if (iotHubClientHandle != NULL)
    {
        ...
        IoTHubTransport_MQTT_Common_Unregister(handleData->deviceHandle);
        if (handleData->isSharedTransport == false)
        {
            ...
            IoTHubTransport_MQTT_Common_Destroy(handleData->transportHandle);
        }
    ...
    else
    {
        ...
        if (messageCallback == NULL)
        {
            ...
            else
            {
                ...
                IoTHubTransport_MQTT_Common_Unsubscribe(handleData->deviceHandle);
        ...
        else
        {
            ...
            {
                if (IoTHubTransport_MQTT_Common_Subscribe(handleData->deviceHandle) == 0)
                {
...
IOTHUB_CLIENT_RESULT IoTHubClient_LL_SendMessageDisposition(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle, MESSAGE_CALLBACK_INFO* message_data, IOTHUBMESSAGE_DISPOSITION_RESULT disposition)
{
    ...
    else
    {
        ...
        result = IoTHubTransport_MQTT_Common_SendMessageDisposition(message_data, disposition);
    }
    return result;
}
...
void IoTHubClient_LL_DoWork(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle)
{
    ...
    if (iotHubClientHandle != NULL)
    {
        ...
        while (client_item != &(handleData->iot_msg_queue)) /*while we are not at the end of the list*/
        {
            ...
            IOTHUB_PROCESS_ITEM_RESULT process_results =  IoTHubTransport_MQTT_Common_ProcessItem(handleData->transportHandle, IOTHUB_TYPE_DEVICE_TWIN, &identity_info);
        ...
        IoTHubTransport_MQTT_Common_DoWork(handleData->transportHandle, iotHubClientHandle);
    }
}
...
IOTHUB_CLIENT_RESULT IoTHubClient_LL_GetSendStatus(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle, IOTHUB_CLIENT_STATUS *iotHubClientStatus)
{
    ...
    else
    {
        ...
        result = IoTHubTransport_MQTT_Common_GetSendStatus(handleData->deviceHandle, iotHubClientStatus);
    }
    ...
    else
    {
        ...
        switch (handleData->methodCallback.type)
        {
            case CALLBACK_TYPE_SYNC:
            {
                ...
                {
                    result = IoTHubTransport_MQTT_Common_DeviceMethod_Response(handleData->deviceHandle, response_id, payload_resp, response_size, result);
                }
                else
                {
                    result = __FAILURE__;
    ...
    else
    {
        ...
        switch (handleData->messageCallback.type)
        {
            ...
            case CALLBACK_TYPE_SYNC:
            {
                ...
                if (IoTHubTransport_MQTT_Common_SendMessageDisposition(messageData, cb_result) != IOTHUB_CLIENT_OK)
                {
...
IOTHUB_CLIENT_RESULT IoTHubClient_LL_SetRetryPolicy(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle, IOTHUB_CLIENT_RETRY_POLICY retryPolicy, size_t retryTimeoutLimitInSeconds)
{
    ...
    else
    {
        ...
        {
            if (IoTHubTransport_MQTT_Common_SetRetryPolicy(handleData->transportHandle, retryPolicy, retryTimeoutLimitInSeconds) != 0)
            {
                result = IOTHUB_CLIENT_ERROR;
                LOG_ERROR_RESULT;
            }
    ...
    else
    {
        ...
        else
        {
            ...
            result =
                ...
                (uploadToBlob_result == IOTHUB_CLIENT_INVALID_ARG) ? IoTHubTransport_MQTT_Common_SetOption(handleData->transportHandle, optionName, value) :
                (uploadToBlob_result == IOTHUB_CLIENT_ERROR) ? IOTHUB_CLIENT_ERROR :
                (IoTHubTransport_MQTT_Common_SetOption(handleData->transportHandle, optionName, value) == IOTHUB_CLIENT_ERROR) ? IOTHUB_CLIENT_ERROR : IOTHUB_CLIENT_OK;
...
IOTHUB_CLIENT_RESULT IoTHubClient_LL_SetDeviceTwinCallback(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle, IOTHUB_CLIENT_DEVICE_TWIN_CALLBACK deviceTwinCallback, void* userContextCallback)
{
    ...
    else
    {
        ...
        if (deviceTwinCallback == NULL)
        {
            ...
            IoTHubTransport_MQTT_Common_Unsubscribe_DeviceTwin(handleData->transportHandle);
        ...
        else
        {
            ...
            if (IoTHubTransport_MQTT_Common_Subscribe_DeviceTwin(handleData->transportHandle) == 0)
            {
    ...
    else
    {
        ...
        if (deviceMethodCallback == NULL)
        {
            ...
            else
            {
                ...
                IoTHubTransport_MQTT_Common_Unsubscribe_DeviceMethod(handleData->transportHandle);
        ...
        else
        {
            ...
            else
            {
                ...
                if (IoTHubTransport_MQTT_Common_Subscribe_DeviceMethod(handleData->deviceHandle) == 0)
                {
                    /*Codes_SRS_IOTHUBCLIENT_LL_12_022: [ Otherwise IoTHubClient_LL_SetDeviceMethodCallback shall succeed and return IOTHUB_CLIENT_OK. ]*/
...
IOTHUB_CLIENT_RESULT IoTHubClient_LL_DeviceMethodResponse(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle, METHOD_HANDLE methodId, const unsigned char* response, size_t response_size, int status_response)
{
    ...
    else
    {
        ...
        if (IoTHubTransport_MQTT_Common_DeviceMethod_Response(handleData->deviceHandle, methodId, response, response_size, status_response) != 0)
        {
