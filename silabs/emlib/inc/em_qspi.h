...
/** QSPI Indirect transfer configuration structure. */
typedef struct
{
  /**
   * Base address for triggering reads from the QSPI internal SRAM and
   * writes to the QSPI internal SRAM. */
  uint32_t                   triggerAddr;

  /** Read watermark, when the number of bytes in the internal read SRAM passes
   * this watermark then an interrupt is generated. */
  uint32_t                   readWatermark;

  /** Write watermark, when the number of bytes in the internal write SRAM
   * falls bellow this watermark then an interrupt is generated. */
  uint32_t                   writeWatermark;
} QSPI_IndirectConfig_TypeDef;
...
/***************************************************************************//**
 * @brief
 *   Start an indirect write transfer.
 *
 * @param[in] qspi
 *   Pointer to QSPI peripheral register block.
 ******************************************************************************/
__STATIC_INLINE void QSPI_IndirectWriteStart(QSPI_TypeDef * qspi)
{
  BUS_RegMaskedSet(&qspi->INDIRECTWRITEXFERCTRL, QSPI_INDIRECTWRITEXFERCTRL_START);
}

/***************************************************************************//**
 * @brief
 *   Cancel an indirect write transfer.
 *
 * @param[in] qspi
 *   Pointer to QSPI peripheral register block.
 ******************************************************************************/
__STATIC_INLINE void QSPI_IndirectWriteCancel(QSPI_TypeDef * qspi)
{
  BUS_RegMaskedSet(&qspi->INDIRECTWRITEXFERCTRL, QSPI_INDIRECTWRITEXFERCTRL_CANCEL);
}

/***************************************************************************//**
 * @brief
 *   Start an indirect read transfer.
 *
 * @param[in] qspi
 *   Pointer to QSPI peripheral register block.
 ******************************************************************************/
__STATIC_INLINE void QSPI_IndirectReadStart(QSPI_TypeDef * qspi)
{
  BUS_RegMaskedSet(&qspi->INDIRECTREADXFERCTRL, QSPI_INDIRECTREADXFERCTRL_START);
}

/***************************************************************************//**
 * @brief
 *   Cancel an indirect read transfer.
 *
 * @param[in] qspi
 *   Pointer to QSPI peripheral register block.
 ******************************************************************************/
__STATIC_INLINE void QSPI_IndirectReadCancel(QSPI_TypeDef * qspi)
{
  BUS_RegMaskedSet(&qspi->INDIRECTREADXFERCTRL, QSPI_INDIRECTREADXFERCTRL_CANCEL);
}
